...
public class Cube {
    ...
    public void draw(Graphics2D g){
        for(int i=0; i<6; i++) {
            if (i == 0) {
                facets[i].draw(g, Color.pink);
            }
            if (i == 1) {
                facets[i].draw(g, Color.yellow);
            }
            if (i == 2) {
                facets[i].draw(g, Color.green);
            }
            if (i == 3) {
                facets[i].draw(g, Color.black);
            }
            if (i == 4) {
                facets[i].draw(g, Color.blue);
            }
            if (i == 5) {
                facets[i].draw(g, Color.red);
            }
        }
    }
}